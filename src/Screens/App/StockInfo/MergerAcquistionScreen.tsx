...
export type MergerAcquisitionScreenProps = CompositeScreenProps<
  StackScreenProps<StockInfoStackNavigatorParamList, RouteStacks.mergerAcquisition>,
  StockInfoStackScreenProps
>

const MergerAcquisitionScreen: FC<MergerAcquisitionScreenProps> = ({ navigation, route }) => {
  const { t } = useTranslation()
  const { Common, Fonts, Gutters, Layout } = useTheme()
  const dispatch = useDispatch()

  return (
    <ScreenBackgrounds screenName={RouteStacks.mergerAcquisition}>
      <Header headerText={t('mergerAcquisition')} onLeftPress={() => navigation.navigate(RouteStacks.stockInfoMain)} withProfile={false} />
...
export default MergerAcquisitionScreen
