...
export type IPONewsDetailScreenProps = CompositeScreenProps<
  StackScreenProps<StockInfoScreenNavigatorParamList, RouteStacks.ipoNewsDetail>,
  StockInfoStackScreenProps
>

const IPONewsDetailScreen: FC<IPONewsDetailScreenProps> = ({ navigation, route }) => {
  const { t } = useTranslation()
  const { Common, Fonts, Gutters, Layout } = useTheme()
  const dispatch = useDispatch()

  return (
    <ScreenBackgrounds screenName={RouteStacks.ipoNewsDetail}>
      <Header headerText={t('ipoNewsDetail')} onLeftPress={() => navigation.navigate(RouteStacks.stockInfoMain)} withProfile={false} />
...
export default IPONewsDetailScreen
